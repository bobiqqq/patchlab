...
def load_user(user_id):
    # Flask-Login хранит id в сессии и по нему восстанавливает пользователя.
...
def register():
    ...
    if request.method == 'POST':
        # Берём данные из обычной HTML-формы регистрации.
        ...
        # Пароль не хранится как текст, model сам сделает hash.
...
def login():
    ...
    if request.method == 'POST':
        # Для входа ищем пользователя по имени и сверяем hash пароля.
        username = request.form.get('username')
        password = request.form.get('password')
...
@dashboard_bp.route('/')
def home():
    # Главная страница для вошедшего пользователя сразу ведёт к задачам.
    if current_user.is_authenticated:
        return redirect(url_for('dashboard.index'))
...
@login_required
def index():
    # История берётся только по текущему пользователю, чтобы не показывать чужие попытки.
...
def task_detail(task):
    # На странице задачи показываем только попытки текущего пользователя.
...
def download_source(task):
    ...
    # source_path хранится относительным путём, здесь превращаем его в полный.
...
def read_source(path, absolute=False):
    # absolute нужен только на случай, если путь уже передали полностью.
...
def create_app():
    ...
    # SQLite-база лежит в data, чтобы не зависеть от Flask instance-папки.
    data_dir = os.path.join(os.getcwd(), 'data')
    os.makedirs(data_dir, exist_ok=True)

    app.config['SECRET_KEY'] = os.getenv('SECRET_KEY', 'super-secret-app-key123')
    if app.config['SECRET_KEY'] == 'super-secret-app-key123':
        print("!!! Не удалось получить env['SECRET_KEY] !!!")
    app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///' + os.path.join(data_dir, 'patchlab.db')
    app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False
    app.config['UPLOAD_FOLDER'] = os.path.join(os.getcwd(), 'uploads')

    # Подключаем расширения к созданному приложению.
    db.init_app(app)
    login_manager.init_app(app)
    login_manager.login_view = 'auth.login'

    # Роуты разделены на простые blueprints: auth, страницы задач и API.
    ...
    with app.app_context():
        # Создаём таблицы и синхронизируем задачи из task_sources.
        db.create_all()
        seed_tasks(db)
